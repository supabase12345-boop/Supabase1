// ===================================
// supabase.js - Elite Capital (نسخة محدثة مع تصحيح الوقت)
// ===================================
...
// ========== نظام المراهنات المبسط (محدث مع تصحيح الوقت) ==========

// إضافة مباراة جديدة (مبسطة)
...
// إضافة رهان جديد (محدث مع تصحيح الوقت)
async function placeBet(betData) {
    try {
        ...
        // نضيف 3 ساعات للمباراة (توقيت بغداد)
        const baghdadMatchTime = new Date(matchDate.getTime() + (3 * 60 * 60 * 1000));
        
        // نضيف 3 ساعات للوقت الحالي (توقيت بغداد)
        const baghdadNow = new Date(now.getTime() + (3 * 60 * 60 * 1000));
        
        // حساب الفرق بالدقائق
        const diffMinutes = (baghdadMatchTime - baghdadNow) / (1000 * 60);
        
        console.log('=== placeBet Debug ===');
        console.log('match_date (UTC):', matchDate);
        console.log('بغداد match:', baghdadMatchTime);
        console.log('بغداد now:', baghdadNow);
        console.log('diffMinutes:', diffMinutes);
        
        // منع الرهان إذا تبقى أقل من 5 دقائق
        if (diffMinutes < 5) {
            throw new Error(`❌ انتهى وقت المراهنة (يُمنع الرهان قبل 5 دقائق من بداية المباراة)`);
        }
        ...
        console.error('خطأ في placeBet:', error);
        return { success: false, error: error.message };
    }
}
...
async function finishMatch(matchId, result, team1Score, team2Score) {
    try {
        ...
        const { data: bets } = await supabaseClient
            .from('user_bets')
            .select('*, users:user_id(balance, total_earned)')
            .eq('match_id', matchId)
            .eq('status', 'pending');
        ...
        for (const bet of bets) {
            ...
            if (isWinner) {
                ...
                await supabaseClient
                    .from('users')
                    .update({
                        balance: (bet.users.balance || 0) + winAmount,
        ...
        await supabaseClient
            .from('matches')
            .update({
                total_paid_amount: (match.total_paid_amount || 0) + totalPaid,
                profit_loss: ((match.profit_loss || 0) + profitLoss),
                updated_at: new Date().toISOString()
            })
            .eq('id', matchId);
...
async function updateBettingStats() {
    try {
        ...
        const totalWonAmount = bets.filter(b => b.status === 'won').reduce((sum, b) => sum + (b.paid_amount || 0), 0);
        const totalLostAmount = bets.filter(b => b.status === 'lost').reduce((sum, b) => sum + (b.amount || 0), 0);
...
async function getMatchStats(matchId) {
    try {
        ...
        const stats = {
            ...
            total_amount: bets.reduce((sum, b) => sum + (b.amount || 0), 0),
            bets_on_1: bets.filter(b => b.bet_value === '1').length,
            amount_on_1: bets.filter(b => b.bet_value === '1').reduce((sum, b) => sum + (b.amount || 0), 0),
            bets_on_x: bets.filter(b => b.bet_value === 'X').length,
            amount_on_x: bets.filter(b => b.bet_value === 'X').reduce((sum, b) => sum + (b.amount || 0), 0),
            bets_on_2: bets.filter(b => b.bet_value === '2').length,
            amount_on_2: bets.filter(b => b.bet_value === '2').reduce((sum, b) => sum + (b.amount || 0), 0),
...
async function cancelBet(betId, userId) {
    try {
        ...
        await supabaseClient
            .from('matches')
            .update({
                total_bets: (bet.matches.total_bets || 0) - 1,
                total_bet_amount: (bet.matches.total_bet_amount || 0) - bet.amount,
                updated_at: new Date().toISOString()
            })
            .eq('id', bet.match_id);
...
// التحقق من حالة المباراة (هل لا يزال مسموح بالرهان)
async function canPlaceBet(matchId) {
    try {
        const { data: match } = await supabaseClient
            .from('matches')
        ...
        if (!match) throw new Error('المباراة غير موجودة');
        if (match.status !== 'upcoming') return { success: true, canBet: false, reason: 'المباراة غير متاحة للمراهنة' };
        
        const matchDate = new Date(match.match_date);
        const now = new Date();
        
        // تحويل إلى توقيت بغداد
        const baghdadMatchTime = new Date(matchDate.getTime() + (3 * 60 * 60 * 1000));
        const baghdadNow = new Date(now.getTime() + (3 * 60 * 60 * 1000));
        ...
        if (diffMinutes < 5) {
            return { success: true, canBet: false, reason: 'انتهى وقت المراهنة (قبل 5 دقائق من المباراة)' };
        }
        
        return { success: true, canBet: true, minutesLeft: Math.floor(diffMinutes) };
    } catch (error) {
        return { success: false, error: error.message };
    }
}
...
console.log('✅ تم تحميل جميع دوال Supabase مع نظام المراهنات وتصحيح الوقت');
