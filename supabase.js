...
async function addMatch(matchData) {
    try {
        ...
        // إضافة خيارات الرهان للمباراة
        const options = matchData.options || [];
...
// تحديث نتيجة المباراة (للمسؤول)
async function updateMatchResult(matchId, result, winningOptionId) {
    try {
        // بدء معاملة
        ...
        // الحصول على جميع الرهانات لهذه المباراة
        ...
        for (const bet of bets) {
            ...
            if (bet.option_id === winningOptionId) {
                ...
                await supabaseClient
                    ...
                    .update({ 
                        ...
                        total_earned: bet.users.total_earned + bet.potential_win
                    })
                    .eq('id', bet.user_id);
                ...
                await addActivity({
                    ...
                    type: 'bet',
                    title: '🏆 فوز في الرهان',
                    description: `فزت بمبلغ ${bet.potential_win}$ في رهان ${match.home_team} vs ${match.away_team}`,
                    amount: bet.potential_win
                });
            } else {
                // تسجيل النشاط للخسارة
                await addActivity({
                    userId: bet.user_id,
                    type: 'bet',
            ...
            await supabaseClient
                ...
                .insert([{
                    ...
                    bet_id: bet.id,
                    action: betStatus,
                    amount: betStatus === 'won' ? bet.potential_win : bet.amount
                }]);
        }
...
async function placeBet(betData) {
    try {
        ...
        // الحصول على خيار الرهان
        const { data: option, error: optionError } = await supabaseClient
            .from('betting_options')
            .select('*, matches(*)')
            .eq('id', betData.optionId)
            .single();
        
        if (optionError) throw optionError;
        
        // التحقق من أن المباراة لا تزال قادمة
        if (option.matches.status !== 'upcoming') {
            throw new Error('لا يمكن الرهان على هذه المباراة');
        }
        
        const potentialWin = betData.amount * option.multiplier;
        ...
        await addActivity({
            ...
            type: 'bet',
            title: '🎯 رهان جديد',
            description: `وضعت رهاناً بمبلغ ${betData.amount}$ على ${option.option_text} (X${option.multiplier})`,
            amount: betData.amount
        });
...
async function getAllBets(status = null) {
    try {
        let query = supabaseClient
            .from('bets')
            .select(`
